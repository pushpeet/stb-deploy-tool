...
import { DiscoverService, getLocalSubnet } from '../services/DiscoverService.js';
import { updateConfig, configExists, writeConfig, readConfig } from '../utils/config.js';
...
export async function discoverCommand(): Promise<void> {
  ...
  if (!net) {
    log.error('Could not detect local network. Are you connected to Wi-Fi or LAN?');
    process.exit(1);
  }
  ...
  log.success(chalk.bold(`Found ${devices.length} STB${devices.length > 1 ? 's' : ''}`));
  log.blank();

  // Display all found devices
  devices.forEach((device, i) => {
    console.log(`  ${chalk.bold(`${i + 1})`)} `);
    console.log(`     Name : ${chalk.cyan(device.hostname)}`);
    console.log(`     IP   : ${chalk.cyan(device.ip)}`);
    console.log(`     SSH  : ${device.verified ? chalk.green('Verified') : chalk.yellow('Unverified (port open)')}`);
    log.blank();
  });
  ...
  if (devices.length === 1) {
    ...
    const { index } = await prompts({
      ...
      choices: devices.map((d, i) => ({
        title: `${d.ip}  ${chalk.dim(d.hostname)}  ${d.verified ? chalk.green('✔ SSH') : chalk.yellow('⚠ unverified')}`,
        value: i,
      })),
    });
    if (index === undefined) {
