...
export async function updateCommand(targetTag?: string): Promise<void> {
  ...
  try {
    await execa("npm", ["install", "-g", "--foreground-scripts", spec], {
      stdio: "inherit",
    });
  } catch (err) {
    log.blank();
    log.error(
      "Update failed. Try clearing the npm cache and retry (do NOT use sudo):",
    );
  ...
  log.blank();
  log.success(
    `Updated to ${tag}. Run ${chalk.bold("stb --version")} to confirm.`,
  );
}
