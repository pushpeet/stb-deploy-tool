...
function getArpIps(subnet: string): string[] {
  try {
    const out = execSync('arp -a 2>/dev/null || ip neigh 2>/dev/null', { encoding: 'utf8' });
...
async function scanIps(ips: string[], port: number, onProgress: (done: number, total: number) => void): Promise<string[]> {
  const open: string[] = [];
  let done = 0;
...
async function verifyStb(ip: string, config: Partial<StbConfig>): Promise<{ hostname: string; verified: boolean }> {
  ...
  try {
    let result;
    if (hasPassword) {
      result = await execa('sshpass', ['-p', password, 'ssh', ...sshArgs], { timeout: 6000 });
    } else {
